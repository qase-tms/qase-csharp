...
{
    public class CreateBaseTestResultTests
    {
        ...
        [Fact]
        public void CreateBaseTestResult_WithBasicTestCase_ShouldCreateTestResult()
        {
            // Arrange
            ...
            result.Relations.Suite.Data[0].Title.Should().Be("TestClass");
        }

        [Fact]
        public void CreateBaseTestResult_WithNestedClassName_ShouldParseSuiteHierarchy()
        {
            // Arrange
            var mockTestCase = CreateMockTestCase("TestMethod", "Namespace.SubNamespace.TestClass", Array.Empty<IParameterInfo>(), Array.Empty<object>());
            var method = _sinkType.GetMethod("CreateBaseTestResult", BindingFlags.NonPublic | BindingFlags.Instance);

            // Act
            var result = method?.Invoke(_sink, new object[] { mockTestCase.Object }) as TestResult;

            // Assert
            result.Should().NotBeNull();
            result.Relations.Suite.Data.Should().HaveCount(3);
            result.Relations.Suite.Data[0].Title.Should().Be("Namespace");
            result.Relations.Suite.Data[1].Title.Should().Be("SubNamespace");
            result.Relations.Suite.Data[2].Title.Should().Be("TestClass");
        }

        [Fact]
        public void CreateBaseTestResult_WithNullTestCase_ShouldThrowArgumentNullException()
        {
            // Arrange
            var method = _sinkType.GetMethod("CreateBaseTestResult", BindingFlags.NonPublic | BindingFlags.Instance);

            // Act & Assert
            var exception = Assert.Throws<TargetInvocationException>(() => method?.Invoke(_sink, new object[] { null }));
            exception.InnerException.Should().BeOfType<ArgumentNullException>();
        }

        [Fact]
        public void CreateBaseTestResult_WithParameters_ShouldExtractParameters()
        {
            // Arrange
            ...
            var arguments = new object[] { "value1", 42 };
            ...
            result.Params["param2"].Should().Be("42");
        }
        ...
        [Fact]
        public void CreateBaseTestResult_ShouldGenerateSignature()
        {
            // Arrange
            var mockTestCase = CreateMockTestCase("TestMethod", "TestClass", Array.Empty<IParameterInfo>(), Array.Empty<object>());
            var method = _sinkType.GetMethod("CreateBaseTestResult", BindingFlags.NonPublic | BindingFlags.Instance);
